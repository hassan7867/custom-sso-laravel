...
class ImageController extends Controller
{
    public function index(Request $request){
        $data = Image::where(['url_kit' => $request->url_kit])->get();
        return json_encode($data);
    }

    public function store(Request $request){
        try{
            $data = Image::where(['url_kit' => $request->url_kit, 'id' => $request->id])->first();
    ...
    public function delete(Request $request){
        try{
            $result = Image::where(['url_kit' => $request->url_kit, 'id' => $request->id])->first()->delete();
            return json_encode(['status' => $result]);
        }catch (\Exception $exception){
            return json_encode(['status' => false, 'message' => 'Failed to save data. There is error on server side!', 'error' => $exception->getMessage()]);
        }
    ...
    public function saveFile(Request $request){
        try {
            ...
            if ($request->hasfile('files')) {
                ...
                $file->move(public_path() . '/kits/' . $request->url_kit . '/images/', $name);
                if (!empty($data->logo)) {
